...
function get_twcode(plan, land_time) {
	...
	var twcode = "[table] [**]Μονάδα[||]Από[||]Στο[||]Χρονος[/**] ";
	var colour = "";
	for (attack in plan) {
		if (plan[attack]['target'] != undefined || plan[attack]['travel_time'] != undefined ||  plan[attack]['type'] != undefined) {
			...
			var a = launch_time.toString('dd/mm/yyyy HH:mm:ss');
			var b = a.indexOf(" GMT");
			var launch_time_final = a.substr(0,b);
			
			twcode += "[*]" + get_troop(plan[attack]['type']) + "[|][coord]" + plan[attack]['attacker'] + "[/coord][|][coord]" + plan[attack]['target'] + "[/coord][|][b][color=" + colour + "]" + launch_time_final + "[/color][/b][/*]";
		}
	}
	twcode += "[/table]";
	return twcode;
